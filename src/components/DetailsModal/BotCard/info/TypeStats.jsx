      ...
      { noDmg.length > 0 && <Grid>
        ...
        >
          {noDmg.map((type, idx) => (
              <Grid
                key={type + idx}
                sx={{ backgroundColor: textColors[type], padding: "5px", display: "flex", alignItems: "center", justifyContent: "center", gap:"4px", borderRadius: "4px", width: "28px"}}
              >
                <img src={typeListSvg[type]} alt={`${type}`} style={{width: "100%"}} />
              </Grid>
          ))}
        </Grid>
      </Grid>}
